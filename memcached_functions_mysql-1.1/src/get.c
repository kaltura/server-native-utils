...
my_bool memc_get_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
  ...
  if (memcached_result_create(container->memc, &container->results) == NULL)
  {
    log_error(args, "memcached_result_create failed");
    return 1;
 }
                ...
                __attribute__ ((unused)) char *error)
{
  ...
  {
    log_error(args, "memcached_mget failed, rc=%d\n", rc);
  }
                      ...
                      __attribute__ ((unused)) char *error)
{
  ...
  if (rc != MEMCACHED_SUCCESS)
  {
    log_error(args, "memcached_mget_by_key failed, rc=%d\n", rc);
  }
