...
static memcached_pool_st* memc_pool= NULL;
...
memcached_return_t recreate_memcache_pool(UDF_ARGS *args)
{
  if (memc_pool != NULL)
  {
    memcached_pool_destroy(memc_pool);
  }
  memc_pool= memcached_pool_create(master_memc, INITIAL_POOL_SIZE, MAX_POOL_SIZE);
  if (memc_pool == NULL)
  {
    log_error(args, "memcached_pool_create failed\n");
                       ...
                       __attribute__ ((unused)) char *error)
{
  ...
  if (master_memc == NULL)
  {
    ...
    log_error(args, "memcached_create failed\n");
    *is_null= 1;
    return 0;
  }
  ...
  {
    pthread_mutex_unlock(&memc_servers_mutex);
    log_error(args, "memcached_servers_parse failed\n");
    *is_null= 1;
    return 0;
  }
  ...
  {
    pthread_mutex_unlock(&memc_servers_mutex);
    log_error(args, "memcached_server_push failed, rc=%d\n", rc);
    *is_null= 1;
    return 0;
  }
  /*
  ...
  */
  ...
  rc = recreate_memcache_pool(args);

  pthread_mutex_unlock(&memc_servers_mutex);

  return ((long long) rc == MEMCACHED_SUCCESS ? 1 : 0);
}
                                    ...
                                    __attribute__ ((unused)) char *error)
{
  ...
  */
  ...
  if (rc != MEMCACHED_SUCCESS)
  {
    log_error(args, "memcached_behavior_set failed, rc=%d\n", rc);
  }
  else
  {
    rc = recreate_memcache_pool(args);
  ...
  return (rc == MEMCACHED_SUCCESS ? 0 : rc);
}
...
/*
  ...
  with memcached.
*/
int memc_get_servers(UDF_ARGS *args, memcached_st **memc)
{
  int retval;
  memcached_st *test;
  memcached_return_t rc;

  *memc= memcached_pool_fetch(memc_pool, NULL, &rc);
  if (MEMCACHED_SUCCESS != rc)
  {
    log_error(args, "memcached_pool_fetch failed, rc=%d\n", rc);
  }
...
/*
  @@brief
  memc_free_servers() releases a memc structure allocated by memc_get_servers.
*/
int memc_free_servers(memcached_st *memc)
{
  memcached_return_t rc;
  
  rc= memcached_pool_release(memc_pool, memc);
  if (MEMCACHED_SUCCESS != rc)
  {
    log_error(NULL, "memcached_pool_release failed, rc=%d\n", rc);
  }
  ...
  */
void fix_null_args(UDF_ARGS *args, int count)
{
	int i;
	for (i = 0; i < count; i++)
	{
...
void log_error(const UDF_ARGS *args, const char *message, ...)
{
  char buf[1024];
  time_t t;
  struct tm *ts;
  ...
  if (args != NULL && args->arg_count > 0 && args->arg_type[0] == STRING_RESULT)
  {
	if (args->args[0] == NULL)
	{
	  strcat(buf, "(null)");
	...
	{
	  strncat(buf, args->args[0], MIN((size_t)args->lengths[0], 100));
	}
  }
